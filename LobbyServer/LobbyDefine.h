...
struct LanRequest
{
	explicit LanRequest(ULONGLONG lanSessionId, USHORT msgType, PacketBufferRef packet, jh_network::IocpServer* lanServer) : m_ullSessionId(lanSessionId), m_usMsgType(msgType), m_pPacket(packet), m_pLanServer(lanServer) {}
	~LanRequest()
	{
